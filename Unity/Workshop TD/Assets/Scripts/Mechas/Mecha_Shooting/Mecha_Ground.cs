...
public class Mechas_Ground : MonoBehaviour
{
    ...
    void UpdateTarget()
    {
        ...
        foreach (GameObject enemy in enemies)
        {
            ...
            {
                Vector3 direction = (enemy.transform.position - Shoot_Point.position).normalized;

                // --- NOUVEAU : Raycast sans filtre + check tag
                if (Physics.Raycast(Shoot_Point.position, direction, out RaycastHit hit, distanceToEnemy))
                {
